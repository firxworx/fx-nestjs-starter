...
This project is an opinionated starter (or "boilerplate") for API's implemented with the [NestJS](https://nestjs.com/) framework powered by [Express](https://expressjs.com/). The code builds on the basic app generated by the nestjs cli and includes additional features and a configuration intended to be applicable to most projects.
...
The purpose of this repo is to provide a monolithic NestJS reference API that's filled with goodies that can help developers save time when starting a new project.
...
- NestJS is powered by Express (vs. optional Fastify) to provide access to the vast ecosystem of Express middleware
- improved `tsconfig.json` vs. nestjs defaults
  - sets `strict: true` to realize the full benefit of TypeScript
  - `lib` includes `es2019` for useful features like `array.flat()`
- configuration is via idomatic @nestjs/config (refer to `src/config`) that reads from the environment
- Swagger/OpenAPI is ready to rock and the the optional nestjs cli plugin provided by `@nestjs/swagger` is enabled
- pino logger via nestjs-pino for an idiomatic logger that logs requests + responses and supports json output

### Modules 

#### AuthModule

  - includes local + JWT + JWT refresh token strategies:
...
#### DatabaseModule
  ...
  - dedicated module powered by TypeORM + postgres, featuring a TypeScript `ormconfig.ts` file with cli support
  - exposes an injectible `QueryUtilsService` that includes helper functions for querying data via TypeORM
  - pagination + filter + sort helpers
    - includes a `PageFilterSortQueryValidationPipe`

#### AwsModule

- implements services using **@aws-sdk v3**
- `AwsSesService` for sending emails
- `AwsS3Service` for storage on S3
- `AwsSnsService` for dispatching notifications including SMS (text messages)
- `AwsEventBridgeService` for emitting events to an event bus
...
Note the scripts in `package.json` and potentially other libraries assume a linux/unix environment. Windows users are advised to run linux on WSL2. The majority of code in this project was created on a PC running Ubuntu under WSL2.
